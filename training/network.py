  ...
  for i, c in enumerate(groups):
    ...
    with tf.variable_scope('Conv_{}'.format(i)):
      ...
      with tf.variable_scope('GatherConvolution'):
        ...
        batch_idx = tf.bitcast(
          tf.reshape(
            tf.tile(
              tf.reshape(tf.range(net_shape[0], dtype=tf.int32), [-1, 1]),
              [1, net_shape[1] * graph_degree * prev_last_out]),
            [net_shape[0], net_shape[1], graph_degree * prev_last_out]), tf.float32)

        neighbour_idx = tf.bitcast(
          tf.reshape(
            tf.tile(tf.reshape(G, [-1, 1]), [1, prev_last_out]),
            [net_shape[0], net_shape[1], graph_degree * prev_last_out]), tf.float32)

        feature_idx = tf.bitcast(
          tf.reshape(
            tf.tile(tf.range(prev_last_out, dtype=tf.int32), [net_shape[0] * net_shape[1] * graph_degree]),
            [net_shape[0], net_shape[1], graph_degree * prev_last_out]), tf.float32)

        # Now we can use this to lookup our actual network
        network_idx = tf.bitcast(
          tf.stack([batch_idx, neighbour_idx, feature_idx], axis=3, name='NetworkIndices'), tf.int32)
