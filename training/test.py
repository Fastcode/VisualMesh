...
def test(sess, network, mesh_type, mesh_size, model_dir, input_path, output_path):
  ...
  data_string = dataset.dataset(
    files[round(len(files) * 0.8):], mesh_type=mesh_type, variants=False, repeat=1, batch_size=10, shuffle=False
  )
