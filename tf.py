...
def get_tree_from_point(mesh, index):
    ...
    points = np.array(sorted(list(points)), dtype=np.int32)
...
def rejection_resample(sess):

    # Minimum chance of being selected is 5%
    min_chance = 0.05

    # Get our data packs
    pack_dir = os.path.join('training', 'packs')
    repack_dir = os.path.join('training', 'resample')
    packs = sorted([f for f in os.listdir(pack_dir) if f.endswith('.bin.gz')])

    for pack in packs[32::4]:
        print('Loading data tree pack {}'.format(pack))
        data = load.pack(os.path.join(pack_dir, pack))
        print('\tloaded')
        ...
        with open(os.path.join(repack_dir, pack[:-3]), 'wb') as f:
